            ...
            for (int i = 0; i < filesQueue.Length; i++)
            {
                ...
                for (int j = 0; j < outputPlatforms.Length; j++)
                {
                    ...
                    if (!string.IsNullOrEmpty(sfxOutputFolder) && Directory.Exists(sfxOutputFolder))
                    {
                        ...
                        {
                            MusXBuild_MusicFile.BuildMusicFile(soundMarkerFilePath, soundSampleDataFilePath, sfxOutputPath, CommonFunctions.GetPlatformLabel(outputPlatforms[j]), CommonFunctions.GetFileHashCode(Enumerations.FileType.MusicFile, Enumerations.Language.English, musicFileData.HashCode), 4);
                        }
