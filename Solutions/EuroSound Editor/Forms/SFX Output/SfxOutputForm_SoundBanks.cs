                ...
                for (int j = 0; j < filesQueue.Length; j++)
                {
                    ...
                    else
                    {
                        ...
                        for (int k = 0; k < outputPlatform.Length; k++)
                        {
                            ...
                            Query.Start();
                            SortedDictionary<string, SFX> sbFileData = sbFunctions.GetSfxDataDict(soundBankData.DataBases, outputPlatform[k], outputLanguage);
                                ...
                                if (!abortQuickOutput)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(outputPath) && Directory.Exists(outputPath))
                                        {
                                            ...
                                            MusXBuild_Soundbank.BuildSoundbankFile(sfxTempFile, sifTempFile, sbfTempFile, ssfTempFile, sfxFilepath, CommonFunctions.GetPlatformLabel(outputPlatform[k]), CommonFunctions.GetFileHashCode(FileType.SoundBank, outputLanguage, soundBankData.HashCode), isBigEndian, 4);
