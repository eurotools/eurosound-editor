        ...
        private void OutputHashCodes(SamplePool samplesList)
        {
            ...
            for (int i = 0; i < outLanguages.Length; i++)
            {
                ...
                for (int j = 0; j < outputPlatform.Length; j++)
                {
                    ...
                    MusXBuild_MusicDetails.BuildMusicDetails(outTmpFilePath, sfxOutputPath, CommonFunctions.GetFileHashCode(FileType.SoundDetails, outLang, 0), CommonFunctions.GetPlatformLabel(outputPlatform[j]), 5);
                }
            }
