...
bool decode(char* inFileName, char* outFileName, u32 outSampleRate)
{
	FILE* f = fopen(inFileName, "rb");
	fseek(f, 0, SEEK_END);
	int inFileSize = ftell(f);
	...
	endianWriteU32Little(&fmtHeader->SamplesPerSec, outSampleRate);
	endianWriteU32Little(&fmtHeader->AvgBytesPerSec, outSampleRate * 2);
...
int main(int argc, char** argv)
{
	if (argc < 4)
	{
		printf("::::::::::::::::::::::IMA ADPCM TOOL::::::::::::::::::::::");
		printf("Encode: <encode> <input file> <output file>\n");
		printf("Decode: <decode> <input file> <sample rate> <output file>\n");
		return 0;
	}
	...
	else if (!stricmp(argv[1], "decode"))
	{
		...
		bool success = decode(argv[2], argv[4], sampleRate);
		printf("%s\n", success ? "success" : "fail");
	}
	else
	{
