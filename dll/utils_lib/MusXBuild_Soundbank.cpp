	...
	void BuildSoundbankFile(char* sfxFilePath, char* sifFilePath, char* sbFilePath, char* ssFilePath, char* outputFilePath, u32 fileHashCode, bool bigEndian)
	{
		...
		if (file.is_open())
		{
			...
			if (ssFile != NULL)
			{
				...
				//Write data
				fileSections.SampleInfoLength = FlipUInt32(ssFileSize, bigEndian);
				file.seekp(positionAligned);
			...
			if (sbFile != NULL)
			{
				...
				if (ssFileSize > 0)
				{
					positionAligned = AlignNumber(file.tellp(), 0x800);
				}
				fileSections.SampleInfoStart = FlipUInt32(positionAligned, bigEndian);
				fileSections.SampleInfoLength = FlipUInt32(sbFileSize, bigEndian);
				file.seekp(positionAligned);
